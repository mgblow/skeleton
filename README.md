...
The app provides a simple model `User`:
...
* `address`: An entity of type `Address` representing the User's postal address.
* `skills`: A `Set<String>`, representing a collection of Strings representing skills the user has.

#### Mapping Entities to JSON

The `User` model is annotated with `@Document`, this enables the models to be saved as RedisJSON documents.
...
For example, saving the above object using a repository (see below) you'll get a JSON document under the Redis key `models.com.cloudcatcity.platform.skeleton.User:01FXD97WTHNYFQEA56YQ27BNQ6`:
...
```java
public interface PeopleRepository extends RedisDocumentRepository<User, String> {
  ...
  // Performs full text search on a user's personal Statement
  Iterable<User> searchByUseralStatement(String text);
...
Let's start with a simple example, a Spring `@Service` which includes `EntityStream` to query for instances of the mapped class `User`:
...
import models.com.cloudcatcity.platform.skeleton.User;
import models.com.cloudcatcity.platform.skeleton.User$;
...
public class PeopleService {
    ...
    // Find all people
    public Iterable<User> findAllPeople(int minAge, int maxAge) {
        return entityStream //
                .of(User.class) //
...
The `EntityStream` is injected into the `PeopleService` using `@Autowired`. We can then get a stream for `User` objects by using `entityStream.of(User.class)`. At this point the stream represents the equivalent of a `SELECT * FROM User` on a relational database. The call to `collect` will then execute the underlying query and return a collection of all `User` objects in Redis.
...
example below, our entity model is `User` therefore we get a meta-model named `User$`. With the meta-model you have access to the operations related to the
...
In this example we also make use of the Streams `sorted` method to declare that our stream will be sorted by the `User$.AGE` in `ASC`ending order.
